    ...
    public static class DomainAutoMapperConfiguration
    {
        public static void AddDomainMappings(IMapperConfigurationExpression cfg)
        {
            ...
            cfg.CreateMap<PaymentEntity, Payment>()
                ...
                .ForMember(dst => dst.CourseType, opt => opt.MapFrom(src => src.CourseType.HasValue ? (CourseType?) src.CourseType.Value : null))
                .ForMember(dst => dst.LearningType, opt => opt.MapFrom(src => src.LearningType.HasValue ? (LearningType?) src.LearningType.Value : null))
